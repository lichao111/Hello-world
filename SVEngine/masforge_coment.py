...
def myTemporaryFile(prefix): #this provides a unique temporary file name only
	return os.path.join(tempfile.tempdir,prefix+next(tempfile._get_candidate_names())+"tmp") #把创建的临时文件的路径和prefix的的路径结合起来
...
class Bunch(object):
	def __init__(self, adict):    #类方法（类函数必须要有self参数，且为第一个）
		self.__dict__.update(adict)#把self和adict（一个字典）链接起来，形成一个字典
...
def merge_file(inlist,output):
	with open(output, 'w') as outfile:
		for infile in inlist:
			shutil.copyfileobj(open(infile), outfile) #chao:shutil.copyfileobj()函数把infile复制到outfile之中

def deovlpvar(dic,maxdeovlp=2):
	nowdeovlp=0; 
	varlist=copy.deepcopy(dic['varlist'])   #copy是浅复制(只拷贝父对象)，deepcopy是深复制(拷贝父对象及其子对象)。   '='  < 'copy'  <  'deepcopy'
	#print varlist
...
def pipecmd(cmd1,cmd2):     #chao:管道命令
	#print >>sys.stderr, cmd1, "|", cmd2
	#run=subprocess.Popen(cmd.split(),env=os.environ,cwd=os.getcwd(),stderr=subprocess.PIPE,stdout=subprocess.PIPE)
	#run_info=run.communicate() 
	run_info=os.system(" ".join([cmd1,"|",cmd2]))# cmd1 | cmd2 管道命令
	return run_info
...
def CountUnique(mylist):
	...
	return h         #chao:h是个字典，key为mylist的值

def isint(s):#Chao:???   int(s):对s取整
	try: int(s); return True;
	except ValueError: return False;

def locatemin(a):# Chao:定位最小值，并且返回他的位置下标
	smallest = min(a)
...
def test_map(a,b):#Chao:把a，b原样输出
	print a,b
...
#---------把gapfile在sizefile的范围内延长，两边各延长edgein的长度-------------------------
	try:
	...
	if runmode==1 or runmode==2: #have meta input, no need of deovlapping
		metatab = CommentedFile(metafile)   #第一行不以#开头的行为metatab,tab:标签
		dic['varlist']=collections.OrderedDict()#有序的字典，可以记住添加的顺序
		dic['varcnt']=0
		varlist,varcnt =makevar(Bunch(dic),metatab)#把metafile转化成varfile
		dic['varlist']=varlist
		dic['varcnt']=varcnt
...
def fq2bam(ffq,bun):#chao：bun是什么？？此函数的作用是用xwgsim生成的read数据输入到bws中生成bam文件
	reffile=bun.reffile.name#很不明白为什么bun可以访问reffile
	nlibrary=bun.parlist.getint('xwgsim', 'nlibrary')
	libnames=json.loads(bun.parlist.get('xwgsim', 'libnames'))#json.dumps()的作用是把str转化成dict，json.loads()的作用是吧dict转化成str
	nprocs=bun.nprocs
	mergemax=bun.mergemax
	oprefix=bun.oprefix
	libbams = [None] * nlibrary#数组，元素都为None，个数为nlibrary个
	for lib in range(0,nlibrary):
		...
		bwasw_cmd2="samtools view -bS -o %s -" % (tbam) #it auto gives bam if not provided    Chao:临时文件中写入了生成的bam文件 ！
...
def fas2bed(fasfile):#Chao：把fasta文件转换成了BED文件    BED：chr	str	end  其中的str都是0！！！1
...
def fas2size(fasfile):#计算出每条ref的条数，一行一个，存储在一个文件(前缀为fasize_)当中,
	sizefile = tempfile.NamedTemporaryFile('w',delete=False,prefix="fasize_")#创建一个有名字的临时文件
	fasseq = pysam.Fastafile(fasfile)
...
def chkvar(varbed):#Chao:检测varfile中是否有重复的区间，如有重复的区间return False！！
	#print varbed.sort().merge()
	#print varbed.sort()
	if varbed.sort().merge().count() == varbed.sort().count(): return True;#count好像和py中独有的，在bedtools中没有
	else: return False

def ppbam(bun,bamfiles):#无用
	return None

def var2wgs(bun):#Chao:没有用到
	hapseq=bun.hapseq
	nprocs=bun.nprocs
	mergemax=bun.mergemax
	oprefix=bun.oprefix
...
def var2fq(bun):#Chao:把varfile转换成有var的文件，关键函数
	hapseq=bun.hapseq
	nprocs=bun.nprocs
	oprefix=bun.oprefix
	parlist=bun.parlist
	...
	for var in varlist.values():
		#print var
		#print [var[4],var[5],var[5]+var[7],var[0],var[1],var[2],var[6],var[8]]
		#CHR ST	ED	VID	MID	FREQ	DELsize	INSformation; adding one base size to ensure flanking regions are separate
		hapvar[var[3]] = hapvar.get(var[3],[]) + [[var[4],var[5],var[5]+var[7]+2*nligation+1,var[0],var[1],var[2],var[7],formatins(var[9])]]
	for hap in hapvar.keys():#hap只有0 和1 两个值，会生成两个bed文件，一个是zahe的，一个chunhe的
		hapvar[hap] = pybedtools.BedTool('\n'.join([ '\t'.join([str(c) for c in x]) for x in hapvar[hap] ]), from_string=True)
		#assert chkvar(hapvar[hap]) == True, "varfile contains self overlaps, which is not allowed"
	for hap in hapvar.keys():
		sizefile = fas2size(hapseq[hap].filename)		 #create genome size file
		hapbed[hap] = hapvar[hap].complement(g=sizefile) #create complementary blocks!Chao:在整个区间（size）中把var删掉剩下的区间\也就是说hapbed和hapvar是互补的！！？？
		hapbed[hap] = hapbed[hap].slop(g=sizefile,b=nligation)#两边各延长nligation
		hapbed[hap] = pybedtools.BedTool('\n'.join([ f[0]+'\t'+str(f[1])+'\t'+str(f[2])+'\tNone\tNone\t0\t0\tNone' for f in hapbed[hap]]),from_string=True)               				#'None none 0 0 none'分别代表vid，mid，delsize，insinformation
		hapvar[hap] = hapvar[hap].cat(hapbed[hap],postmerge=False)
...
#hapiter.append([hap,hapfas,nligation,parlist,reffile,nploid,idx,total,feat,mergemax])
	hap=hapit[0]; hapfas=hapit[1]; nligation=hapit[2]; parlist=hapit[3]; reffile=hapit[4]; nploid=hapit[5]; bed=hapit[8]; mergemax=hapit[9]#bed就是之前的hapvar。
...
def file2var(bun):
	...
	varlist=collections.OrderedDict()#Chao:现在varlist是一个空的有顺序的字典
	for row in ifile: 
		varlist[row[0]]=row;
...
def makevar(bun,metatab,fas=False):         # Chao:change meta to var!!!   Y of N??  Y
	...
	trunks,stretches = blocktrunks(trunks,gaptab)				
	#f.write(str(trunks))
	#f.close()
...
def maketid(stretches,varstretch):#此函数的作用是返回够长度的区间下标，还有就是除去要生成var之后剩下的区间，要生成的那些区间记为0
	#print stretches
	#print varstretch
	#print len(stretches)
